...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    ignore_weight = 0
    bg_weight = 1 # background
    car_weight = 2  # Car
    ped_weight = 10
    cyc_weight = 20

    not_ignore_mask = \
        tf.to_float(tf.equal(scaled_labels, 0)) * bg_weight + \
        tf.to_float(tf.equal(scaled_labels, 1)) * car_weight + \
        tf.to_float(tf.equal(scaled_labels, 2)) * ped_weight + \
        tf.to_float(tf.equal(scaled_labels, 3)) * cyc_weight + \
