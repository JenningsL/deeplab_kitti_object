...
_FEATURE_OUTPUT_NAME = 'FeatureMaps'
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    raw_predictions = tf.identity(
        tf.cast(predictions[common.OUTPUT_TYPE+'_softmax'], tf.float32),
        _RAW_OUTPUT_NAME)
    # Crop the valid regions from the predictions.
    semantic_predictions = tf.slice(
        raw_predictions,
        [0, 0, 0, 0],
        [1, resized_image_size[0], resized_image_size[1], -1])
    ...
    print('before resize: ', semantic_predictions.shape)
    semantic_predictions = _resize_label(semantic_predictions, image_size)
    semantic_predictions = tf.identity(semantic_predictions, name=_OUTPUT_NAME)
    # feature map
    print(predictions['feature_map'])
    feature_map = _resize_label(predictions['feature_map'], image_size)
    feature_map = tf.identity(feature_map, _FEATURE_OUTPUT_NAME)
