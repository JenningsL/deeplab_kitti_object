...
def predict_labels(images, model_options, image_pyramid=None):
  ...
  Returns:
    A dictionary with keys specifying the output_type (e.g., semantic
      ...
      over channels). Each prediction has size [batch, height, width].
  """
  outputs_to_scales_to_logits, feature_map = multi_scale_logits(
      images,
      model_options=model_options,
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    if model_options.prediction_with_upsampled_logits:
      ...
      # softmax
      predictions[output+'_softmax'] = tf.nn.softmax(logits, axis=3)
    else:
      ...
      # softmax
      softmax_results = tf.nn.softmax(logits, axis=3)
      softmax_results = tf.image.resize_nearest_neighbor(
          softmax_results,
          tf.shape(images)[1:3],
          align_corners=True,
          name='resize_prediction_softmax')
      predictions[output+'_softmax'] = softmax_results

  predictions['feature_map'] = feature_map
  return predictions
...
def multi_scale_logits(images,
  ...
  for image_scale in image_pyramid:
    ...
    outputs_to_logits, feature_map = _get_logits(
        scaled_images,
        updated_options,
    ...
    if len(image_pyramid) == 1:
      ...
      return outputs_to_scales_to_logits, feature_map
